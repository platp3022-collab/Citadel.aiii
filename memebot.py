...
try:
    import scout as wallet_scout           # сам находит эти кошельки
except ImportError:
    wallet_scout = None                    # type: ignore[assignment]
...
CONFIG: dict[str, Any] = {
    ...
    "trade": {
        ...
        # Свежую позицию переоцениваем часто, иначе стоп −18% срабатывает на дне:
        # монета проскакивает его между редкими опросами. На истории это давало
        # средний минус −55% при стопе −18%. См. trader.loop().
        "poll_seconds": 30,          # базовый цикл, когда свежих позиций нет
        "fast_poll_seconds": 5,      # частый опрос, пока позиция моложе fast_poll_minutes
        "fast_poll_minutes": 12,
        ...
        "rules": {
            # Скальп под высокий винрейт для обычной сделки. Цель — чтобы
            # монета, которая ВООБЩЕ пыхнула, закрывалась в плюс: снимаем
            # большую часть рано (60% на +15%) и сразу двигаем стоп в безубыток.
            # После этого сделка уже зелёная — остаток либо едет дальше на
            # трейлинге, либо выходит около входа, но не в минус.
            # Плата за винрейт: иксов на всю позицию не будет (60% снято рано),
            # и монету, которая не дошла до +15%, стоп −18% всё равно закроет
            # в минус — винрейт задаётся тем, как часто монета берёт +15%
            # раньше −18%, а это держит гейт confirm на входе.
            "метрики": {
                "take_profit_pct": 0.0,        # потолка нет — хвост может ехать
                "stop_loss_pct": -18.0,
                "scale_out_at_pct": 15.0,      # снимаем рано — фиксируем плюс
                "scale_out_pct": 60.0,         # 60%: сделка сразу в реализованном плюсе
                "breakeven_after_scale": True, # и стоп сразу в безубыток
                "keep_after_scale_pct": 2.0,   # остаток не отдаём ниже +2% → сделка зелёная
                "trail_after_scale_pct": 12.0, # хвост бережём плотным трейлингом
                "trailing_after_pct": 25.0,
                "trailing_stop_pct": 15.0,
                "timeout_minutes": 12.0,
            },
            "кимчи": {
    ...
    # и начисляет очки тем, кто в них сидит. Набрал очков — попал в слежку.
    "scout": {
        "enabled": True,
        "sweep_minutes": 12,
        "judge_after_minutes": 60,   # через сколько судить монету
        "winner_multiple": 2.0,      # рост в 2 раза — выстрел
        "min_wins": 3,               # столько попаданий → в слежку
        "max_tracked": 40,
    },
...
class Bot:
    def __init__(self, session: aiohttp.ClientSession, dry: bool = False):
        ...
        self.scout = None
        if (wallet_scout is not None and self.wallets is not None
                and cfg("scout.enabled", True)):
            self.scout = wallet_scout.WalletScout(session, self.wallets,
                                                  conf=cfg("scout") or {})
    ...
    async def scout_loop(self) -> None:
        """Разведка: кто оказался в выстреливших монетах."""
        if self.scout is None:
            return
        await self.scout.loop(self.stop_event)
