...
CONFIG: dict[str, Any] = {
    ...
    "trade": {
        ...
        "rules": {
            # Лесенка фиксации для обычной сделки (плейбук скальперов: банчить
            # частями на +50/+100/+200, а не одной продажей). Смысл — чаще
            # видеть реализованный плюс: снимаем по 25% на каждом рывке, после
            # первого транша низ уже не отдаём, остаток едет на трейлинге.
            # Лосс это НЕ лечит — монета, не дошедшая до +30%, всё равно уйдёт
            # в стоп −18%; ладдер лишь снимает прибыль с тех, что рвутся вверх.
            "метрики": {
                "take_profit_pct": 0.0,        # потолка нет — остаток едет за иксами
                "stop_loss_pct": -18.0,
                "scale_out_at_pct": 30.0,      # первый транш уже на +30%
                "tranche_pct": 25.0,           # снимаем по 25% на каждом шаге
                "tranche_step_pct": 30.0,      # шаг между траншами +30% (30/60/90…)
                "breakeven_after_scale": True, # после первого транша защищаем низ
                "keep_after_scale_pct": 12.0,  # остаток не отдаём ниже +12%
                "trailing_after_pct": 50.0,
